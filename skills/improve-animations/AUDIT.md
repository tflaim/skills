...
The eight audit categories, what to hunt for in each, and the exact target values to cite in findings and plans. Recreated from [emilkowalski/skills](https://github.com/emilkowalski/skills) (MIT, © Emil Kowalski), distilling his design engineering philosophy ([emilkowal.ski](https://emilkowal.ski/)). Never approximate a value that appears here — copy it.
...
Hunt for: animations on keyboard-initiated actions, command palettes with open/close transitions (Raycast has none — correct), decorative motion on constantly-hit list items or hover states. The strongest fix is often **delete the animation**.
...
- **Never `scale(0)`** — nothing real appears from nothing. Target: `scale(0.9–0.97)` + `opacity: 0`.
...
- Gesture-driven motion uses springs — they carry velocity when interrupted.
...
Hunt for: `@keyframes` on toasts/toggles/rapidly-triggered UI, gesture handlers tweening with fixed-duration keyframes, drags without velocity-based dismissal (dismiss on `Math.abs(distance)/elapsedMs > ~0.11`, not distance thresholds alone), hard stops at drag boundaries instead of rising friction.
...
- **Framer Motion `x`/`y`/`scale` shorthands are not hardware-accelerated** — main-thread rAF, drops frames under load. Target: the full transform string, `animate={{ transform: "translateX(100px)" }}`.
...
- CSS (and WAAPI) beat rAF-based JS under load — CSS for predetermined motion, JS/springs for dynamic and gesture-driven motion.
...
- Motion matches the product's personality — playful can bounce, a dashboard stays crisp. Mismatched personality across components is a finding.
- Curves and durations live as shared tokens. Five hand-typed cubic-beziers that almost match is a consolidation finding.
...
- Spatially-connected UI (a panel appearing from a trigger) with no motion explaining where it came from.
